...
def quad_kl_div(pi, ref):
    return (
        torch.sum(pi) * torch.sum(ref * log_ent(pi / ref))
...
def l2_distortion(pi, Cx, Cy):
    ...
    C = torch.sum(
        torch.einsum("ij,jl->il", Cx, pi) * torch.einsum("ij,jl->il", pi, Cy)
    )
    return A + B - 2 * C
...
def gw_cost(pi, a, Cx, b, Cy, rho, blur):
    if blur == 0.0:
        return (
            l2_distortion(pi, Cx, Cy)
            + rho * quad_kl_div(torch.sum(pi, dim=1), a)
        ...
        return (
            ...
            + rho * quad_kl_div(torch.sum(pi, dim=1), a)
            + rho * quad_kl_div(torch.sum(pi, dim=0), b)
            + blur * quad_kl_div(pi, a[:, None] * b[None, :])
        )
