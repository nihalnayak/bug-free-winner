...
def main(x, y, args):
    ...
    np.save(os.path.join(DIR_PATH, f'data/large_coupling_type_{args.type}'\
                         f'_dim_{args.dim}_k_{args.k}_e_{args.e}_seed_{args.seed}.npy'),
            scot.coupling[0])

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--type", help='type of the dimensionality reduction technique')
    parser.add_argument("--dim", help="name of the dataset", type=int)
    parser.add_argument("--k", help="number of neighbours in the knn graph", type=int, default=50)
    parser.add_argument("--e", help="epsilon value in the SCOT algorithm", type=int, default=0.0005)
