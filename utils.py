...
def unit_normalize(data, norm="l2", bySample=True):
    ...
    assert norm in [
        ...
        "l2",
        "max",
    ], "Norm argument has to be either one of 'max', 'l1', or 'l2'."
...
def get_graph_distance_matrix(
    data, num_neighbors, mode="connectivity", metric="correlation"
):
    """
    ...
    shortestPath_data = dijkstra(
        csgraph=csr_matrix(graph_data),
        directed=False,
